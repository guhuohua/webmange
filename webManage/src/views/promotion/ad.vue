<template>
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入商品标题" v-model="listQuery.name">
      </el-input>
      <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入关键字" v-model="listQuery.content">
      </el-input>
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="商品链接" prop="link">
      </el-table-column>

      <el-table-column align="center" label="商品标题" prop="name">
      </el-table-column>

      <el-table-column align="center" label="推荐内容" prop="content">
      </el-table-column>

      <el-table-column align="center" label="推荐图片" prop="url">
        <template slot-scope="scope">
          <img :src="scope.row.url" width="80" v-if="scope.row.url"/>
        </template>
      </el-table-column>

      <el-table-column align="center" label="推荐位置" prop="position">
        <template slot-scope="scope">
          <el-tag :type="scope.row.position === 1 ? 'success' : 'error' " v-if="scope.row.position === 1">首页轮播</el-tag>
          <el-tag :type="scope.row.position === 2 ? 'success' : 'error' " v-if="scope.row.position === 2">精品推荐</el-tag>
          <el-tag :type="scope.row.position === 3 ? 'success' : 'error' " v-if="scope.row.position === 3">热门推荐</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="商品编码" prop="link">
        ...
        <el-form-item label="广告位置" prop="position">
          <el-select v-model="dataForm.position" placeholder="请选择">
            ...
            <el-option label="热门推荐" :value="2"></el-option>
            <el-option label="精品推荐" :value="3"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        link: [{ required: true, message: '内容不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    uploadUrl: function(response) {
      this.dataForm.url = response.res.url
    },
    createData() {
