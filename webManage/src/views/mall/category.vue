<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="类目ID" prop="gtId">
      </el-table-column>
      ...
      <el-table-column align="center" property="iconUrl" label="类目图标">
        <template slot-scope="scope">
          ...
          <div v-if="!scope.row.iconUrl">-</div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="级别" prop="parentId"
        ...
        <template slot-scope="scope">
          <el-tag :type="scope.row.parentId === 0 ? 'primary' : 'info' ">{{scope.row.parentId === 0 ? '一级分类' : '二级分类'}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="父类目" prop="parentId" v-if="dataForm.level === 'L2'">
          <el-select v-model="dataForm.parentId" :placeholder="请选择">
            <el-option v-for="item in catL1" :key="item.name" :label="item.name" :value="item.gtId">
            </el-option>
          </el-select>
        </el-form-item>
  ...
  methods: {
    ...
    getCatL1() {
      listCatL1().then(response => {
        this.catL1 = response.data.res
        if (this.catL1.length > 0) {
          this.dataForm.parentId = this.catL1[0].name
        }
      })
    },
    handleFilter() {
    ...
    uploadIconUrl: function(response) {
      this.dataForm.iconUrl = response.res.url
    },
    uploadPicUrl: function(response) {
      this.dataForm.picUrl = response.res.url
    },
    createData() {
