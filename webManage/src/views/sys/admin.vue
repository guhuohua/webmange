<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="管理员头像" prop="photo">
        <template slot-scope="scope">
          <img :src="scope.row.photo" width="40" v-if="scope.row.photo"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="管理员头像" prop="photo">
          <el-upload class="photo-uploader" :headers="headers" :action="uploadPath" list-type="picture-card" :show-file-list="false" accept=".jpg,.jpeg,.png,.gif" :on-success="uploadAvatar">
			      <img v-if="dataForm.photo" :src="dataForm.photo" class="photo">
...
export default {
  ...
  methods: {
    ...
    uploadAvatar: function(response) {
      this.dataForm.photo = response.res.url
    },
    handleCreate() {
      this.resetForm()
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const filterVal = ['id', 'username', 'photo']
