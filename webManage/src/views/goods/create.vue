  ...
  <div class="app-container calendar-list-container">
    ...
    <el-card class="box-card">
      ...
      <el-form :rules="rules" ref="goods" :model="goods" label-width="150px">
        <el-form-item label="商品编号" prop="goodsSn">
          <el-input placeholder="请输入内容" v-model="goods.goodsFlag" class="input-with-select">
            <el-button slot="append" icon="el-icon-edit-outline" @click="randomFlag">随机编号</el-button>
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="商品原价" prop="oriPrice">
          <el-input v-model="goods.oriPrice" placeholder="0.00">
        ...
        <el-form-item label="是否热卖" prop="hot">
          <el-radio-group v-model="goods.hot">
        ...
        <el-form-item label="库存数量">
          <el-input v-model="goods.repertory" placeholder="0"></el-input>
        </el-form-item>
        ...
        <el-form-item label="商品分类">
          <el-cascader expand-trigger="hover" :options="typeList" @change="handleCategoryChange"></el-cascader>
        </el-form-item>

        <!--<el-form-item label="二级分类">-->
          <!--<el-select v-model="goods.brandId">-->
            <!--<el-option v-for="item in childList" :key="item.value" :label="item.label" :value="item.value">-->
            <!--</el-option>-->
          <!--</el-select>-->
        <!--</el-form-item>-->
    ...
    <!--<el-card class="box-card">
      ...
      <el-table :data="products">
        <el-table-column property="value" label="商品规格">
          <template slot-scope="scope">
        ...
        <el-table-column property="price" width="100" label="商品售价">
        </el-table-column>
        <el-table-column property="number" width="100" label="商品数量">
        </el-table-column>
        <el-table-column property="url" width="100" label="商品图片">
          <template slot-scope="scope">
      ...
      <el-dialog title="设置商品" :visible.sync="productVisiable">
        <el-form ref="productForm" :model="productForm" status-icon label-position="left" label-width="100px"
                 style='width: 400px; margin-left:50px;'>
          <el-form-item label="商品规格列" prop="specifications">
...
<script>
  import { publishGoods, typeList, getRandomGoodsSn } from '@/api/goods'
  ...
  export default {
    ...
    data() {
      return {
        ...
        typeList: [],
        childList: [],
        goods: { picUrl: '', imgUrls: [], repertory: 0, goodsFlag: '' },
        specVisiable: false,
        ...
        attributes: [],
        rules: {
          goodsFlag: [{ required: true, message: '商品编号不能为空', trigger: 'blur' }],
    ...
    methods: {
      init: function() {
        typeList().then(response => {
          this.typeList = response.data.res
        })
      },
      ...
      uploadPicUrl: function(response) {
        ...
        this.goods.picUrl = response.res.url
      },
      ...
      handleGalleryUrl(response, file, fileList) {
        if (response.status === 1) {
          this.goods.imgUrls.push(response.res.url)
        }
      },
      handleRemove: function(file, fileList) {
        for (var i = 0; i < this.goods.imgUrls.length; i++) {
          // 这里存在两种情况
          // 1. 如果所删除图片是刚刚上传的图片，那么图片地址是file.response.res.url
          ...
          if (file.response === undefined) {
            ...
            url = file.response.res.url
          }

          if (this.goods.imgUrls[i] === url) {
            this.goods.imgUrls.splice(i, 1)
          }
        }
      },
      ...
      uploadSpecPicUrl: function(response) {
        this.specForm.picUrl = response.res.url
      },
      ...
      uploadProductUrl: function(response) {
        this.productForm.url = response.res.url
      },
      handleProductEdit() {
      ...
      randomFlag() {
        getRandomGoodsSn().then(res => {
          console.log('res', res)
          if (res.data.status === 1) {
            this.goods.goodsFlag = res.data.res
          }
        })
      }
    }
