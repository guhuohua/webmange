<template>
  ...
  <el-card class="box-card">
    ...
    <el-form :rules="rules" ref="goods" :model="goods" label-width="150px">
        <el-form-item label="商品编号" prop="goodsFlag">
          <el-input v-model="goods.goodsFlag" disabled></el-input>
        </el-form-item>
        ...
        <el-form-item label="详情图片">
          <el-upload :action='uploadPath' :headers="headers" :limit='5'
                     multiple accept=".jpg,.jpeg,.png,.gif" :file-list="galleryFileList"
        ...
        <el-cascader expand-trigger="hover" :options="typeList" v-model="categoryIds" @change="handleCategoryChange"></el-cascader>
        </el-form-item>
...
<script>
import { detailGoods, editGoods, typeList } from '@/api/goods'
  ...
  methods: {
    init: function() {
      ...
      detailGoods(goodsId).then(response => {
        this.goods = response.data.res.goods
        this.specifications = response.data.res.specifications
        this.products = response.data.res.products
        this.attributes = response.data.res.attributes
        this.categoryIds = response.data.res.categoryIds

        this.galleryFileList = []
        for (var i = 0; i < this.goods.imgUrls.length; i++) {
          this.galleryFileList.push({
            url: this.goods.imgUrls[i].url
          })
        }
      })
    ...
    handleEdit: function() {
      const finalGoods = {
        ...
        galleryFileList: this.galleryFileList
      }
    ...
    handleGalleryUrl(response, file, fileList) {
      if (response.status === 1) {
        this.galleryFileList.push(response.res.url)
      }
    },
